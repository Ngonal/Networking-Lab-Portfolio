...
| 2 | TDR-enabled cable tester indicates a short on the far end of the link. Local RJ45 connector is seated in `SW1`'s `Fa0/1`, remote end found disconnected from `PC1`'s `Fa0`. End user reports accidental disconnection of the RJ45 connector. | Reconnected cable to `PC1`'s `Fa0` | No change; `Fa0/1` LEDs remain unilluminated | <img src="Elements/Step2.png"> |
...
| 4 | Both hosts appear to have link connectivity according to their interface LEDs. | Tested with `ping` via Windows Command Prompt | Communication successful | <img src="Elements/Step4.png"> |
