...
| 2 | `show interfaces status` on `SW4` reveals `Gi1/0/1` is hard-coded to 100 Mbps — confirms the speed mismatch that caused the initial link failure | Issued `speed auto` on `SW4`'s `Gi1/0/1`; confirmed with `show interfaces Gi1/0/1 status` | Both interface speeds set to autonegotiation; configuration normalized on both switches | <img src="Elements/step2.png"> |
...
## Bonus Tips
### Tip #1 - Hard-coded interface speeds are a red flag. Interfaces speeds are expected to be determined by autonegotiation speed by default — an explicit speed setting immediately suggests intentional or erroneous misconfiguration:
