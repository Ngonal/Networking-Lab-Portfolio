...
| 2 | Local RJ45 connector is seated in `SW1`'s `Fa0/1`, TDR device indicates an open circuit on the far end of the link, remote end found disconnected from `PC1`'s `Fa0`, the end user reports accidental disconnection of the RJ45 connector | Reconnected cable to `PC1`'s `Fa0` to close circuit | No change; `Fa0/1` LEDs remain unilluminated | <img src="Elements/step2.png"> |
